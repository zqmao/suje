...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    protected static final int MonitorMessage = 20;
	...
	public static class MonitorEvent{
    	...
    	public MonitorEvent(MonitorEventType type) {
			super();
			this.type = type;
		}
		...
		public void setType(MonitorEventType type) {
			this.type = type;
		}
    }
    ...
    final public void sendMonitorMessage(Object data) {
    	if(data != null && data instanceof MonitorEvent ){
    		sendMessage(obtainMessage(MonitorMessage, data));
    	}else{
    		Log.e(LOG_TAG, "Unknown Event " + data);
    	}
    }
    ...
    protected void handleMessage(Message message) {
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if(isCancle()){
                	Log.d(LOG_TAG, "SUCCESS_MESSAGE has cancled");
                	return;
                }
            ...
            case FAILURE_MESSAGE:
                response = (Object[]) message.obj;
                if(isCancle()){
                	Log.d(LOG_TAG, "FAILURE_MESSAGE has cancled");
                	return;
                }
            ...
            case FINISH_MESSAGE:
            	if(isCancle()){
                	Log.d(LOG_TAG, "FINISH_MESSAGE has cancled");
                	return;
                }
                onFinish();
                break;
            ...
            case MonitorMessage:
            	response = (Object[]) message.obj;
            	if(response != null && response.length == 1){
            		MonitorEvent event = (MonitorEvent) response[0];
            		if(event.getType() == MonitorEventType.OvertimeBreak){
            			onFailure(0, null, null, (Throwable) response[0]);
            		}
            	}else{
            		Log.e(LOG_TAG, "ERROR_CANCEL_MESSAGE didn't get enough params");
            	}
                break;
        }
    }
